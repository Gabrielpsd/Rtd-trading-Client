...
OPCAOB3 alocaOpcoes(char * tag)
{
  ...
  for(ptr = tag;(ultimatag = strstr(ptr,TAGTABELA)); ptr = ultimatag + 1)
  {
    ...
    opcao.vencimento = substring(tag,posicoesTds[7]+16,posicoesTds[8]-4);
...
{
  char * url;
  url = (char *) malloc(BUFFER * sizeof(char));
  strcpy(url,"https://bvmf.bmfbovespa.com.br/cias-listadas/Titulos-Negociaveis/DetalheTitulosNegociaveis.aspx?or=res&cb=");
  strcat(url,acao);
  strcat(url,"&tip=I&idioma=pt-BR");
  return url;
}
...
void salvandoNoBancoDeDados(OPCOES opcoes)
{
  ...
  sqlite3_open("opcoes.db", &db);

  sql = "CREATE TABLE IF NOT EXISTS OPCOES("  \
        "ACAO TEXT NOT NULL," \
  ...
  sql = "DELETE FROM OPCOES WHERE ACAO = (?);";
  sqlite3_prepare_v2(db, sql, -1, &stmt, 0);
  sqlite3_bind_text(stmt, 1, opcoes.acao, -1, SQLITE_STATIC);
  sqlite3_step(stmt);
  sqlite3_finalize(stmt);

  for(contador = 0; contador < opcoes.quantidade; contador++)
  {
    sql = "INSERT INTO OPCOES (ACAO, CODIGO, VENCIMENTO, PRECO) VALUES (?,?,?,?);";
    sqlite3_prepare_v2(db, sql, -1, &stmt, 0);
    sqlite3_bind_text(stmt, 1, opcoes.acao, -1, SQLITE_STATIC);
    sqlite3_bind_text(stmt, 2, opcoes.opcoes[contador].codigo, -1, SQLITE_STATIC);
    sqlite3_bind_text(stmt, 3, opcoes.opcoes[contador].vencimento, -1, SQLITE_STATIC);
    sqlite3_bind_double(stmt, 4, opcoes.opcoes[contador].preco);
    sqlite3_step(stmt);
    sqlite3_finalize(stmt);
  }
